...
public function TotalOrder() {  
    ...
    if(!is_null($out)){   // yes data
        ...
        public function MostUser($num) {  
            $query = "SELECT `user`.`id`,`user`.`email`, SUM(`tbl_order`.`quantity`) as quantity, SUM(`tbl_order`.`price`) as tot_price
            FROM tbl_order
            INNER JOIN `user` ON `user`.`id` = `tbl_order`.`fk_userId`
            GROUP BY `user`.`email`
            ORDER BY SUM(`tbl_order`.`price`) DESC
            LIMIT {$num}";  
                        $result = $this->db->select($query);
                        if($result){   // yes data
                            $data = $result->fetch_all(MYSQLI_ASSOC);
